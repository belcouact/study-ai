...
// Function to add Ghibli-style floating animations
function initGhibliAnimations() {
    // Add subtle floating animation to cards and containers
    const elementsToAnimate = document.querySelectorAll('.poem-display, .word-card, .container');
    ...
    elementsToAnimate.forEach(element => {
        ...
        setInterval(() => {
            const randomY = Math.random() * 10 - 5; // Between -5px and 5px
            const randomRotate = Math.random() * 0.5 - 0.25; // Between -0.25deg and 0.25deg
    ...
    // Add subtle color shift to sidebar
    const sidebar = document.querySelector('.left-panel');
    if (sidebar) {
        setInterval(() => {
            const hue = 180 + Math.random() * 20 - 10; // Teal base with slight variations
            sidebar.style.filter = `hue-rotate(${hue - 180}deg)`;
            
            setTimeout(() => {
                sidebar.style.filter = 'hue-rotate(0)';
            }, 2000);
        }, 10000);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Only initialize animations if not on a mobile device
    if (window.innerWidth > 768) {
        initGhibliAnimations();
    }
    
    // Add cloud particles in the background
    createClouds();
});
...
function createClouds() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        cloud.style.cssText = `
            ...
            filter: blur(10px);
            z-index: -1;
            animation: floatCloud ${20 + Math.random() * 30}s linear infinite;
        `;
        
        contentArea.appendChild(cloud);
    }
}

// Add keyframes animation for cloud movement
...
styleSheet.textContent = `
    ...
    .ghibli-atmosphere {
        ...
        width: 100%;
        height: 100%;
        background: radial-gradient(circle at top, rgba(248, 243, 230, 0.3), transparent);
        pointer-events: none;
        z-index: -1;
    }
