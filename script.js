            ...
            mutations.forEach((mutation) => {
                if (mutation.addedNodes.length) {
                    mutation.addedNodes.forEach((node) => {
                        if (node.classList && (node.classList.contains('word-card') || 
                            node.classList.contains('poem-display') || 
...
// Add subtle animation effects to Ghibli theme elements
document.addEventListener('DOMContentLoaded', function() {
    ...
    const addFloatingEffect = () => {
        ...
        cards.forEach(card => {
            ...
            if (!card.dataset.hasFloatingEffect) {
                card.dataset.hasFloatingEffect = 'true';
                ...
                card.addEventListener('mouseleave', () => {
                    ...
                    // Add subtle bounce back animation
    ...
    // Add observer to watch for new content
    const contentObserver = new MutationObserver((mutations) => {
        mutations.forEach(mutation => {
            if (mutation.addedNodes.length) {
                // Add floating effect to any new cards
                addFloatingEffect();
            }
        });
    });
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        const moveX = (pageX / windowWidth) * 10 - 5;
        const moveY = (pageY / windowHeight) * 10 - 5;
        
        // Select all decorative pseudo-elements (we'll target their parent elements)
        const containers = document.querySelectorAll('.container, .word-card, .poem-display');
        
        containers.forEach(container => {
            // Apply subtle movement to pseudo-elements via their parent
...
// Preserve the existing script.js functionality
