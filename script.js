...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleTabSwitch(containerType) {
        ...
        if (containerType === 'qa' && qaContainer && contentArea) {
            ...
            // Sync sidebar values to test page controls
            syncTestControls();
            
            // Set up the generate questions button
...
// ... existing code ...
    ...
    // Function to sync sidebar values to test page controls
    function syncTestControls() {
        ...
        // Get test page selects
        const subjectSelectTest = document.getElementById('subject-select-test');
        const semesterSelectTest = document.getElementById('semester-select-test');
        const difficultySelectTest = document.getElementById('difficulty-select-test');
        const questionCountSelectTest = document.getElementById('question-count-select-test');
        
        // Populate subject options based on school and grade
        if (schoolSelect && gradeSelect && subjectSelectTest) {
            // Clear existing options
            while (subjectSelectTest.options.length > 0) {
                subjectSelectTest.remove(0);
            }
            
            // Get subjects for the current school and grade
            const school = schoolSelect.value;
            const grade = gradeSelect.value;
            const subjects = getSubjectsForSchoolAndGrade(school, grade);
            ...
            subjects.forEach(subject => {
                ...
                option.value = subject;
                option.textContent = subject;
                subjectSelectTest.appendChild(option);
            });
            
            // If sidebar subject is in the list, select it
            if (subjectSelectSidebar && subjectSelectSidebar.value) {
                const sidebarValue = subjectSelectSidebar.value;
                for (let i = 0; i < subjectSelectTest.options.length; i++) {
                    if (subjectSelectTest.options[i].value === sidebarValue) {
                        subjectSelectTest.selectedIndex = i;
                        break;
                    }
                }
            }
        ...
        // Sync other values
        if (semesterSelectSidebar && semesterSelectTest) {
            semesterSelectTest.value = semesterSelectSidebar.value;
        }
        
        if (difficultySelectSidebar && difficultySelectTest) {
            difficultySelectTest.value = difficultySelectSidebar.value;
        }
        
        if (questionCountSelectSidebar && questionCountSelectTest) {
            questionCountSelectTest.value = questionCountSelectSidebar.value;
        }
    }
    
    // Function to get subjects for a school and grade
    function getSubjectsForSchoolAndGrade(school, grade) {
        let subjects = ['语文', '数学'];
        ...
        if (school === '小学') {
            ...
            subjects = ['语文', '数学', '英语', '物理', '化学', '历史', '地理', '生物', '政治'];
        }
    ...
    async function handleGenerateQuestionsClick() {
        console.log('Handling generate questions click');
        
        // Get test configuration
        const schoolSelect = document.getElementById('school-select-sidebar');
        const gradeSelect = document.getElementById('grade-select-sidebar');
        const subjectSelect = document.getElementById('subject-select-test');
        const semesterSelect = document.getElementById('semester-select-test');
        const difficultySelect = document.getElementById('difficulty-select-test');
        const questionCountSelect = document.getElementById('question-count-select-test');
        
        if (!schoolSelect || !gradeSelect || !subjectSelect || !semesterSelect || 
            !difficultySelect || !questionCountSelect) {
            showSystemMessage('无法获取测验配置信息', 'error');
            return;
        }
        ...
        if (!school || !grade || !subject || !semester || !difficulty || !questionCount) {
            showSystemMessage('请完成所有测验配置', 'warning');
            return;
        }
        
        console.log(`Generating ${questionCount} ${difficulty} ${subject} questions for ${school} ${grade} ${semester}`);
        ...
        if (!loadingIndicator) {
            ...
            loadingIndicator.innerHTML = `
                <div class="spinner"></div>
                <p>正在生成${school}${grade}${semester}${subject}${difficulty}难度的${questionCount}道题目...</p>
            `;
            
            const createContainer = document.getElementById('create-container');
            if (createContainer) {
                createContainer.appendChild(loadingIndicator);
            }
        } else {
            loadingIndicator.style.display = 'flex';
        }
        ...
        try {
            ...
            const prompt = `请为${school}${grade}${semester}${subject}课程生成${questionCount}道${difficulty}难度的选择题。
            ...
            请确保题目符合${school}${grade}学生的认知水平，难度为${difficulty}。
            题目应该覆盖${semester}的重要知识点，并且具有教育意义。
            ...
            // Call the API
            const apiResponse = await fetchAIResponse(prompt);
            console.log('API response received for test questions');
            ...
            if (questions.length > 0) {
                ...
                // Store questions in global state
                window.questionState = {
                    questions: questions,
                    currentIndex: 0,
