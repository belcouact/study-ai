...
// Function to initialize form layout
function initializeFormLayout() {
    console.log('Initializing form layout');
    
    // Add a generate button to the form
    const formContainer = document.querySelector('.test-configuration');
    if (formContainer) {
        // The button is now added directly in HTML
        console.log('Test configuration container found');
    }
    
    // Get school and grade selects
    const schoolSelect = document.getElementById('school-select-sidebar');
    const gradeSelect = document.getElementById('grade-select-sidebar');
    
    // Get subject select from the test page (not sidebar)
    const subjectSelect = document.getElementById('subject-select');
    
    // Populate grade options based on selected school
    if (schoolSelect && gradeSelect) {
        ...
        schoolSelect.addEventListener('change', function() {
            ...
            // If subject select exists, update its options too
            if (subjectSelect) {
    ...
    // Populate subject options based on selected school
...
// Function to populate subject options in the new test page select
function populateSubjectOptions(school) {
    const subjectSelect = document.getElementById('subject-select');
    if (!subjectSelect) return;
...
function handleGenerateQuestionsClick() {
    ...
    const schoolSelect = document.getElementById('school-select-sidebar');
    const gradeSelect = document.getElementById('grade-select-sidebar');
    const subjectSelect = document.getElementById('subject-select'); // Updated to use the new select
    const semesterSelect = document.getElementById('semester-select'); // Updated to use the new select
    const difficultySelect = document.getElementById('difficulty-select'); // Updated to use the new select
    const questionCountSelect = document.getElementById('question-count-select'); // Updated to use the new select
    
    // Validate form inputs
    if (!schoolSelect || !gradeSelect || !subjectSelect || !semesterSelect || !difficultySelect || !questionCountSelect) {
        showSystemMessage('表单输入不完整', 'error');
        return;
    }
    ...
    if (!school || !grade || !subject || !semester || !difficulty || !questionCount) {
        showSystemMessage('请选择所有必填项', 'warning');
        return;
    }
    ...
    const prompt = `请为${school}${grade}学生生成${questionCount}道${subject}${semester}考试选择题，难度为${difficulty}。
    
    题目格式要求：
    1. 每道题必须有四个选项(A, B, C, D)
    2. 必须标明正确答案
    3. 必须提供详细的解析
    4. 如有数学公式，请使用LaTeX格式，例如 $x^2$
    ...
    请确保题目难度适合${school}${grade}学生的水平，符合${subject}${semester}课程内容。`;
