...
document.querySelector('.lang-toggle').addEventListener('click', function() {
    ...
    contentSections.forEach(section => {
        if (section.classList.contains('en')) {
            section.classList.remove('en');
            section.classList.add('zh');
            this.textContent = 'Switch to English';
        } else {
            section.classList.remove('zh');
            section.classList.add('en');
            this.textContent = '切换到中文';
        }
        section.style.display = section.classList.contains('en') ? 'block' : 'none';
    });
});

// Initialize language state when modal opens
function initializeLanguageState() {
    ...
    document.querySelector('.lang-toggle').textContent = '切换到中文';
}
