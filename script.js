...
// Function to parse CSV with escaped commas
...
async function fetchInspirationalQuotes() {
    try {
        ...
        const allQuotes = data.split('\n')
            .filter(line => line.trim()) // Remove empty lines
            .map(parseCSVLine);
        
        // Get 10 random quotes
        currentQuotes = getRandomQuotes(allQuotes, 10);
        displayQuote(currentIndex);
    } catch (error) {
        console.error('Error loading quotes:', error);
        document.querySelector('.quote-content').innerHTML = 'Failed to load quotes';
    }
}
...
function displayQuote(index) {
    ...
    quoteContent.innerHTML = `
        <div class="english">${quote.english}</div>
        <div class="chinese">${quote.chinese}</div>
    `;
}
...
// ... existing code ...
document.addEventListener('DOMContentLoaded', fetchInspirationalQuotes);
