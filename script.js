...
function handleGenerateQuestionsClick() {
    ...
    if (!schoolSelect || !gradeSelect || !subjectSelect || !semesterSelect || !difficultySelect || !questionCountSelect) {
        ...
        showSystemMessage('配置选项未找到，请刷新页面重试', 'error');
        return;
    }
    ...
    console.log(`Generating questions for ${school} ${grade} ${subject} ${semester} ${difficulty} ${questionCount}`);
    ...
    const prompt = `请为${school}${grade}的学生生成${questionCount}道${subject}${semester}${difficulty}难度的选择题，每道题有4个选项(A, B, C, D)，并且只有一个正确答案。
    ...
    请确保题目难度适合${school}${grade}的学生，内容符合${subject}${semester}的教学大纲。`;
